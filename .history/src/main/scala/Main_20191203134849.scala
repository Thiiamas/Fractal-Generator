...
object Main extends JFXApp {
  ...
  toolBar.items.addAll(reset,buttonTest,Repeat,Stop,slider,DrawFromPoint)
  ...
  val selectPoint = new Button("SelectPoint")
  selectPoint.onAction = (event:ActionEvent) => {
    isSelectPoint = !isSelectPoint
  }
  mainPane.onMouseClicked = (event:MouseEvent) => {
    if (isSelectPoint) {
      val point = new Complex(event.x, event.y)
      val complexPoint: Complex = calculus.pixelToComplex(point._1,point._2,dim._1,dim._2,dimension.getX._1,dimension.getX._2,dimension.getY._1,dimension.getY._2)
  ...
  fromComplex.items.addAll(inputReal,inputImaginary,drawnFrom,selectPoint,returnMan)
