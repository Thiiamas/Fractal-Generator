...
/*     if (savedSlider != slider.getValue) {
  ...
  mainPane.onMouseReleased = (event:MouseEvent) => {
    ...
    val startPoint: Complex = calculus.pixelToComplex(event.x,event.y,dim._1,dim._2,dimension.getX._1,dimension.getX._2,dimension.getY._1,dimension.getY._2)
    dimension = new Dimensions((zX,event.x),(event.y,zY))
