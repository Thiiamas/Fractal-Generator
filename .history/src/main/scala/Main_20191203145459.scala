...
object Main extends JFXApp {
  ...
  triangleScene.root = trianglePane
  //for Mandelbrot
  val mandelbrotScene: Scene = new Scene
  var iteration: Int = 25
  val mandelbrot = new Mandelbrot(dim)  //initialiser directement
  val dimTextX = new TextArea("X : ("+dimension.getX._1 + ","+dimension.getX._2 +")")
  val dimTextY = new TextArea("Y : ("+dimension.getY._1 + ","+dimension.getY._2 +")")
  val displayDim = new scalafx.scene.layout.VBox()
  tt.children.addAll(dimTextX,dimTextY)
  val manLayout = new BorderPane(mandelbrot.getCanvas)
  manLayout.setBottom(new Vbo (10,dimTextX,dimTextY))
...
/*     if (savedSlider != slider.getValue) {
  ...
  mainPane.onMouseReleased = (event:MouseEvent) => {
    if (zoom.isSelected()){
      ...
      dimension = new Dimensions((startPoint.getReal,endPoint.getReal),(endPoint.getImaginary,startPoint.getImaginary))
    }
    if (mandelbrot.vName == "Man") mandelbrot.display("Mandelbrot",dimension,slider.getValue().toInt)
    else if (mandelbrot.vName == "Others") mandelbrot.display("Fractal",dimension,slider.getValue().toInt,new Complex(inputReal.getText().toDouble, inputImaginary.getText().toDouble))
  ...
  mainPane.children.addAll(toolBar)
