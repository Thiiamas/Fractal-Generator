...
object ScalaFXHelloWorld extends JFXApp {
  ...
  val timer = AnimationTimer(t => {
    ...
    mainPane.onScroll = (event: ScrollEvent) => {
      //si deltaY > 0 on zoom
      val zoomFactor = 1.5
      var currentScale = dimension.XX/zoomFactor
      val point = (event.x,event.y)
      val complexPoint = Calculus.pixelToComplex
      val newX = (event.x. - currentScale/2,event.x + currentScale/2)
      val newY = (event.y - currentScale/2,event.y + currentScale/2)
